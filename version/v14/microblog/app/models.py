...
class Post(db.Model):
    ...
    language = db.Column(db.String(5)) #first run AddLangV14 to create the field
...
def AddLangV14(): #call once, then add in class Post ...
  from sqlalchemy import create_engine
  engine = create_engine('sqlite:///app.db')
  #language = db.Post.Column(db.String(5))
  engine.execute = ('alter table post add column language String(5)') 
  #engine.execute('select * from post')
  #for row in result:
